			...
			if (game.gameState == 4 || game.gameState == 3 || game.gameState == 2 ) {
				...
				if (!Game.level.isMobsAlive() && !Game.level.isBossesAlive()) {
					if (!Game.level.getStatus().equals("continue")) {
						Game.level.addMob(new ContinueButton(Game.GAME_MAX_WIDTH-300, Game.GAME_MAX_BORDER * .49f, 0, Game.level));
						resetPlayers(Game.level.getPlayers());
						
					} 
					
					if (!Game.level.getStatus().equals("mainmenu")) {
						Game.level.addMob(new MainMenuButton(Game.GAME_MAX_WIDTH-300, Game.GAME_MAX_BORDER * .66f, 1, Game.level));
						resetPlayers(Game.level.getPlayers());
					}
				...
				if (!Game.level.isPlayersAlive() && Game.level.isBossesAlive()) {
					...
					if (!Game.level.getStatus().equals("mainmenu")) {
						Game.level.addMob(new MainMenuButton(Game.GAME_MAX_WIDTH-300, Game.GAME_MAX_BORDER * .66f, 0, Game.level));
						resetPlayers(Game.level.getPlayers());
					}
					
					if (!Game.level.getStatus().equals("restart")) {
						Game.level.addMob(new RestartButton(Game.GAME_MAX_WIDTH-300, Game.GAME_MAX_BORDER * .49f, 1, Game.level));
						resetPlayers(Game.level.getPlayers());
